...
// add your code here -- you must use four instances of the full adder "fulladd", defined below
	...
	always @ (*)
		begin
			...
			fulladd bit0(.x(a[1]), .y(b[1]), .cin(c[0]), .sum(result[1]), .cout(c[1]));
			fulladd bit0(.x(a[2]), .y(b[2]), .cin(c[1]), .sum(result[2]), .cout(c[2]));
			fulladd bit0(.x(a[3]), .y(b[3]), .cin(c[2]), .sum(result[3]), .cout(c[3]));
			result[4] : c[3];
		end
endmodule
